...
The note block begins with a `%%CW_CARD_START%%` comment and ends with a `%%CW_CARD_END%%` comment. Both comments reference the same card title / ID stored in `uid`. The card file holds the full payload plus every note reference, including `note_path`, `start_line`, and `end_line`.
...
```json
{
  ...
  "memory_tricks": {
    ...
    "qa_pairs": [
      ...
      {
        "q": "Polymorphism lets you treat different types through a _____ interface.",
        "a": "A shared or common interface."
      },
      {
        "q": "The two main kinds are ___-time (overloading) and ___-time (overriding) polymorphism.",
        "a": "Compile-time and run-time polymorphism."
      }
    ]
  },
  "referenced_in": [
...
```json
{
  ...
  "memory_tricks": {
    ...
    "qa_pairs": [
      ...
      {
        "q": "After resolving a rebase conflict you run `git rebase _____` to continue.",
        "a": "`--continue`."
      },
      {
        "q": "Never rebase commits that are already on a _____ branch.",
        "a": "A shared branch."
      }
    ]
  },
...
```json
{
  ...
  "memory_tricks": {
    ...
    "qa_pairs": [
      ...
      {
        "q": "Merge Sort always runs in O(_____) time but requires O(_____) extra space.",
        "a": "O(n log n) time and O(n) extra space."
      },
      {
        "q": "Quick Sort degrades to O(_____) when pivot selection is consistently poor.",
        "a": "O(n²)."
      }
    ]
  },
...
```json
{
  ...
  "memory_tricks": {
    ...
    "qa_pairs": [
      ...
      {
        "q": "REST may cause ___-fetching because the server decides the response shape.",
        "a": "Over-fetching."
      },
      {
        "q": "GraphQL sends all queries to a _____ endpoint.",
        "a": "A single endpoint."
      }
    ]
  },
...
```json
{
  ...
  "memory_tricks": {
    ...
    "qa_pairs": [
      ...
      {
        "q": "`0 == '0'` evaluates to _____ because == performs type _____.",
        "a": "True, because `==` performs type coercion."
      },
      {
        "q": "`0 === '0'` evaluates to _____ because the types do not _____.",
        "a": "False, because the types do not match."
      }
    ]
  },
  "referenced_in": [
...
```json
{
  ...
  "memory_tricks": {
    ...
    "qa_pairs": [
      ...
      {
        "q": "A pure function produces the same _____ for the same input every time.",
        "a": "Output or return value."
      },
      {
        "q": "Pure functions have no _____ _____.",
        "a": "Side effects."
      }
    ]
  },
...
| Field | Description |
|---|---|
| `uid` | Unique card title / ID, matches the `%%CW_CARD_START%%` comment |
...
| `memory_tricks.qa_pairs` | Array of `{ q, a }` objects for active recall, including prompts that use `_____` blanks directly in `q` |
